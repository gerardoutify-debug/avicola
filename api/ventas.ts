    ...
    if (req.method === 'GET') {
      ...
      const mapRows = (rows: any[], type: string) => {
        ...
        return rows.map((row: any, idx: number) => ({
          idPedido: row[0] || `${type === 'Carcasa' ? 'C' : 'E'}-${idx+1}`,
          fecha: row[1] || '',
          proveedor: row[2] || '',
          origen: row[3] || '',
          loteCodigo: row[4] || '',
          ...
          tipoVenta: row[9] || (type === 'Carcasa' ? 'Carcasa' : 'Entero'),
          cliente: row[10] || '',
      ...
      const ventasCarcasa = mapRows(rowsCarcasa, 'Carcasa');
      const ventasEntero = mapRows(rowsEntero, 'Entero');
